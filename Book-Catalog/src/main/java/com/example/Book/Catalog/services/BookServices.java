...
public class BookServices {
    ...
    @Cacheable(cacheNames = "bookSearch", key = "T(java.lang.String).format('%s:%s:%s',#term,#page,#size")
    public Page<BookRepository.ShopBookView> search(String term , int page , int size){
        log.info("DB HIT: searching term="+term+" page="+page+" size="+size);
        String q = (term == null ? "": term);
        return repository.findByTitleContainingIgnoreCase(q, PageRequest.of(page,size));
    }
